...
Разделение логики и отображения является одним из принципов разработки с ORCHID.
Одним из элементов отображения являются "Layouts", это макеты которые могут отображаться как:
- Таблицы
- Колонки
- Строки
- Графики
